...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'name.required' => 'Product name has to be filled',
        ]);
        $input = $request->all();
        // dd($input);
        // logika image
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($product) {
            ...
            ], ['price.integer' => 'Price has to be an number!']);
            ...
            if ($image = $request->file('image')) {
                ...
                if ($product->image && file_exists($target . $product->image)) {
                    unlink($target . $product->image);
                }

                $img_name = date('YmdHis') . "." . $image->getClientOriginalExtension();
                $image->move($target, $img_name);
                $input['image'] = $img_name; // Save the new image name
            } else {
